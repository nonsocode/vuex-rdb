...
const getRelationshipSchema = (entityDef) =>
  ...
  _remove(items) {
    ...
    switch (true) {
      ...
      case relationship instanceof HasManyRelationship: {
        schema.findByIds(itemIds).forEach((item) => (item[relationship.foreignKey] = null));
        break;
      }
    }
  }
...
function cacheDefaults(model, overrides = {}) {
  Object.entries(getConstructor(model)._fields).forEach(([key, definition]) => {
    ...
    model._caches[getCacheName(definition instanceof Relationship)][key] =
      (_a = overrides[key]) !== null && _a !== void 0 ? _a : definition.default;
  });
}
